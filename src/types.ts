...
interface DatabaseTypeDef {
  type: string; // Enum name for database types
  values?: number[] | undefined; // Optional array of integer/boolean values to populate the enum
  defaultValue?: any; // Default value if a specific one is not provided in schema
}
...
export function parseSchemaToTypes(schemaMap: Record<string, DatabaseTypeDef>): Type[] {
 ...
 */
interface AlchemySchema {
  [key: string]: DatabaseTypeDef; // Column name -> value in C/C# style struct definition
}
